"use strict";
const User_1 = require("../angular/app/model/User");
...
passport.use(new FacebookStrategy({
    ...
    let user = new User_1.User('', name, profile.emails[0].value, 'password', 0, 0);
...
passport.use(new GitHubStrategy({
    ...
    let user = new User_1.User('', name, email, 'password', 0, 0);
    handleCallbackUser(user, cb);
}));
...
let handleCallbackUser = (user, cb) => {
    app_database_1.databaseConnection.db.collection('users')
        .findOne({
        ...
        if (r === null) {
            ...
            //1user.passwordHash = user.password;
            delete user._id;
